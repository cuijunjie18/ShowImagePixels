        ...
        stream.on('end', () => {
            ...
            try {
                ...
                if ((buffer[0] === 0x49 && buffer[1] === 0x49 && buffer[2] === 0x2A && buffer[3] === 0x00) ||
                    ...
                    const readU16 = le ? (o) => buffer.readUInt16LE(o) : (o) => buffer.readUInt16BE(o);
                    const readU32 = le ? (o) => buffer.readUInt32LE(o) : (o) => buffer.readUInt32BE(o);
                ...
                reject(new Error(`Failed to parse image: ${e.message}`));
            }
        });
    });
}
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand('showimagepixels.showPixels', async (uri) => {
        if (!uri) {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`${fileName}: ${dimensions.width} x ${dimensions.height} pixels`);
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to read image dimensions for ${fileName}: ${error.message}`);
        }
    });
